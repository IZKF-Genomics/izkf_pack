...
def resolve(ctx) -> str:
    common = _load_common()
    value = common.latest_output(ctx, "salmon_dir")
    if isinstance(value, str) and value.strip():
        return value.strip()
    raise RuntimeError(
        "salmon_dir could not be resolved because no upstream nfcore salmon_dir output was found in the current project"
    )
