...
say() {
  printf '[scrna_annotate_scanvi_reference] %s\n' "$*"
}

say "starting scANVI reference label transfer"
say "workspace: ${script_dir}"
say "results: ${results_dir}"

if [ -z "${REFERENCE_H5AD:-}" ]; then
  say "REFERENCE_H5AD is not set; using config/dataset.toml reference_h5ad"
fi
...
say "  ${results_dir}/tables/scanvi_cell_predictions.csv"
say "  ${results_dir}/tables/scanvi_cluster_summary.csv"
