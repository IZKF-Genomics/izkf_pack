...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        source_root = root / "fastq"
        target_root = root / "archive"
        manifest_dir = root / "manifests"
        run_dir = source_root / "240101_FASTQ_TEST"
        run_dir.mkdir(parents=True)
        target_root.mkdir()
        (run_dir / "report.txt").write_text("ok\n", encoding="utf-8")
        (run_dir / "sample.fastq.gz").write_text("fq\n", encoding="utf-8")
        ...
        assert manifest["workflow"] == "archive_fastq"
        assert manifest["records"][0]["status"] == "completed"
        assert not run_dir.exists()
        assert (target_root / "240101_FASTQ_TEST" / "report.txt").exists()
        assert not (target_root / "240101_FASTQ_TEST" / "sample.fastq.gz").exists()
    return 0
