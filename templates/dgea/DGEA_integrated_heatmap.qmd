...
single_comparison_file <- params$single_comparison_file
if (is.null(single_comparison_file) || identical(single_comparison_file, "")) {
  single_comparison_file <- stat_files[[1]]
}
if (!single_comparison_file %in% stat_files) {
  warning(
    "single_comparison_file not found in stat files: ", single_comparison_file,
    ". Skipping the single-comparison section."
  )
  single_comparison_file <- NULL
}
...
if (is.null(single_comparison_file)) {
  cat("No valid single comparison file configured. Set params$single_comparison_file to highlight one comparison.")
} else {
  filtered_genes <- filter_genes(stat_files, top_n = params$top_n_single)
  target_genes <- filtered_genes[[single_comparison_file]]
  generate_heatmap(
    target_genes,
    fontsize_row = 4,
    main = paste0("Top ", params$top_n_single, " DE genes in ", single_comparison_file),
    show_rownames = TRUE
  )
}
```
