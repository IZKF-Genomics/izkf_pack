...
is_standard_nfcore_rnaseq <- function(x) normalize_application(x) == "nfcorernaseq"
...
if (is.null(OrgDb) && (go || gsea)) {
  orgdb_issue <- paste0(
    "GO/GSEA requested, but no OrgDb annotation package is available for organism '",
    organism,
    "'. GO and GSEA were disabled for this report."
  )
  warning(orgdb_issue)
  go <- FALSE
  gsea <- FALSE
...
show_go_up <- FALSE
show_go_down <- FALSE
tx2gene_file <- if (nzchar(params$tx2gene_file)) {
  params$tx2gene_file
} else if (file.exists(file.path(params$salmon_dir, "tx2gene.tsv"))) {
  file.path(params$salmon_dir, "tx2gene.tsv")
} else {
  file.path(params$salmon_dir, "salmon.merged.tx2gene.tsv")
}
stopifnot(
...
if (is_standard_nfcore_rnaseq(params$application)) {
...
`r if (!is.null(orgdb_issue)) paste0("> **Note:** ", orgdb_issue) else ""`
...
# Select significant genes or fall back to the top 20 most variable genes
significant_genes <- res %>%
  filter(padj < config$cutoff_adj_p) %>%
  arrange(padj)

if (nrow(significant_genes) < 20) {
  # Fewer than 20 significant genes, use the top 20 most variable genes instead
  gene_variances <- apply(normalized_counts, 1, var)
  top_var_gene_ids <- names(sort(gene_variances, decreasing = TRUE))[1:20]
  significant_genes <- res %>%
    filter(gene_id %in% top_var_gene_ids) %>%
    mutate(gene_id = factor(gene_id, levels = top_var_gene_ids)) %>%
    arrange(gene_id) %>%
    mutate(gene_id = as.character(gene_id))
  heatmap_title <- "Heatmap of Top 20 Varianced Genes (log10 normalized counts)"
} else {
  heatmap_title <- paste0(
    "Heatmap of Significant Genes (log10 normalized counts, N=",
    length(significant_genes$gene_id),
    ")"
  )
}
...
if (nrow(significant_genes) < 20) {
  ...
  significant_genes <- res %>%
    ...
    arrange(gene_id) %>%
    mutate(gene_id = as.character(gene_id))
  heatmap_title <- "Row-scaled Heatmap of Top 20 Varianced Genes"
} else {
  heatmap_title <- paste0(
    "Row-scaled Heatmap of Significant Genes (N=",
    length(significant_genes$gene_id),
    ")"
  )
}
...
```{r go-up-emapplot, echo=go, results="asis", warning=FALSE, message=FALSE, eval=show_go_up}
emap_up <- tryCatch(
  {
    emapplot(
  ...
  error = function(e) {
    cat("> Enrichment map skipped: ", conditionMessage(e), "\n\n", sep = "")
    NULL
  }
)
if (!is.null(emap_up)) {
...
```{r go-down-emapplot, echo=go, results="asis", warning=FALSE, message=FALSE, eval=show_go_down}
emap_down <- tryCatch(
  {
    emapplot(
